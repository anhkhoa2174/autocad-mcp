...
async def entity(
    ...
    # --- Tier 1 validation guards (hard rules) ---
    _BOUNDS_MIN, _BOUNDS_MAX = -10000, 100000
    if operation == "create_line" and None not in (x1, y1, x2, y2):
        if abs(x1 - x2) > 0.1 and abs(y1 - y2) > 0.1 and not data.get("allow_diagonal", False):
            return _json({"error": "Wall must be axis-aligned (y1==y2 or x1==x2). Set data.allow_diagonal=true if intentional."})
    if operation == "create_rectangle" and None not in (x1, y1, x2, y2):
        if abs(x2 - x1) < 100 or abs(y2 - y1) < 100:
            return _json({"error": "Degenerate rectangle (width or height < 100mm)"})
    for _v in (x1, y1, x2, y2):
        if _v is not None and not (_BOUNDS_MIN <= _v <= _BOUNDS_MAX):
            return _json({"error": f"Coordinate {_v} out of bounds [{_BOUNDS_MIN}, {_BOUNDS_MAX}]"})

    # --- Tier 2 guards: layer whitelist + door bbox overlap ---
    _LAYER_WHITELIST_PREFIX = ("CW-", "A-", "F-", "E-", "L-", "ANNO-", "GRID", "HATCH", "DEFPOINTS", "XREF", "0")
    if operation.startswith("create_") and layer and not data.get("allow_layer", False):
        if not any(layer == p or layer.startswith(p) for p in _LAYER_WHITELIST_PREFIX):
            return _json({"error": f"Layer '{layer}' not in whitelist. Use CW-*/A-*/F-*/E-*/L-*/ANNO-*/GRID/HATCH/0 or set data.allow_layer=true."})

    # NOTE: Door INSERT bbox overlap check moved to skill side via
    # scripts/layout.py mode "check_door_overlap" (Tier 3 server-side
    # check requires backend door cache populator — deferred for now).
    # Skill MUST run pre-flight door check before entmake polyline.
...
async def annotation(
    ...
    # --- Tier 1 validation guard: reject literal \uXXXX in text ---
    if operation == "create_text":
        _text = data.get("text", "")
        if "\\u" in _text:
            return _json({"error": f"Use real Unicode char, not \\uXXXX escape (got: {_text[:40]})"})
